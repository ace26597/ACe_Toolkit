...
| Date | Change |
|------|--------|
| 2026-02-09 | **Session Persistence** - Chat: `WorkspaceChatSession` DB model, sessions list/get/rename/persist endpoints, messages stored as JSON, dedicated persist endpoint (no Claude SDK call). Terminal: removed 24h auto-delete (10-year expiry), removed DB/filesystem cleanup, migration extends existing sessions, transcript download endpoint, recording download query param. **Audit fixes:** removed double persistence (server saves cost/turns/claude_session_id only, frontend sends complete message snapshot via /persist), added cache eviction for idle sessions (>1h, skips locked), fixed DELETE ownership DB fallback, added pagination (limit/offset default=50 max=200) to terminal session listing endpoints |
