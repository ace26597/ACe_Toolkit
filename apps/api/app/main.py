...
from app.routers import auth, diagrams, export, notes, ai, projects, charts, research_chat, logs, medresearch
...
async def lifespan(app: FastAPI):
    ...
    # Start periodic MedResearch session cleanup task
    from app.core.medresearch_manager import medresearch_manager
    from app.routers.medresearch import cleanup_expired_sessions
    from app.core.database import AsyncSessionLocal

    async def periodic_medresearch_cleanup():
        """Cleanup expired MedResearch sessions every hour"""
        while True:
            try:
                await asyncio.sleep(3600)  # Wait 1 hour
                ...
                async with AsyncSessionLocal() as db:
                    deleted_db = await cleanup_expired_sessions(db)
                logger.info(f"MedResearch cleanup: {deleted_fs} filesystem, {deleted_db} database entries")
            except Exception as e:
    ...
    # Cleanup: shutdown MedResearch processes
    try:
        logger.info("Shutting down MedResearch processes...")
...
app.include_router(medresearch.router, prefix="/medresearch", tags=["MedResearch"])
