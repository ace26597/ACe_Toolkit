...
export default function MermaidPage() {
  ...
  `);
    ...
    useEffect(() => {
        if (!currentId && diagrams.length > 0) return; // Don't autosave to 'current' if we haven't selected one
    ...
    const renderDiagram = useCallback(async () => {
        ...
        const txtColor = theme === 'dark' ? '#e5e7eb' : '#374151';
...
<body>
    ...
    <script type="module">
        ...
        window.addEventListener('mousedown', (e) => {
            if (e.target.closest('.node') || e.target.closest('.edgeLabel')) return; // Allow clicking nodes
            isDragging = true;
        ...
        try {
            ...
            // Escape backticks for the template literal below
            ...
            // Smart Hint for misplaced frontmatter
        ...
        const handleMessage = (e: MessageEvent) => {
            if (e.data?.type === 'MERMAID_EDIT' && e.data?.text) {
                ...
                if (newText !== null && newText !== oldText) {
                    setCode(prev => {
                        // Naive replace for whiteboard MVP
                        // Escape regex chars in oldText
    ...
    const loadDiagram = (diagram: Diagram) => {
        ...
        // setShowSidebar(false); // User requested sidebar to act as panel, so maybe keep it open?
    };

    const newDiagram = () => {
        const defaultCode = \`graph TD
    A[Start] --> B[End]\`;
        // Create immediately so it appears in list
    ...
    }, [isDragging, onMouseMove, stopResize]);
    ...
    return (
        <div className="flex flex-col h-screen overflow-hidden bg-gray-100 dark:bg-gray-900">
            ...
            <header className="flex-none h-14 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-4 z-10 relative">
                <div className="flex items-center space-x-3">
                    ...
                    <button
                        ...
                        className={\`px-3 py-1 text-sm rounded transition-colors \${showSidebar ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-200' : 'bg-gray-100 dark:bg-gray-700 dark:text-gray-300'}\`}
                    >
            ...
            <div className="flex-1 flex overflow-hidden relative">
                {/* Sidebar (Panel Mode) */}
                <div 
                    className={\`flex-none bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transition-all duration-300 ease-in-out overflow-hidden \${showSidebar ? 'w-72' : 'w-0'}\`}
                >
                            ...
                            <div className="space-y-2">
                                {diagrams.map((d) => (
                                    <div
                                        ...
                                        className={\`p-3 rounded cursor-pointer transition-colors border \${d.id === currentId ? 'bg-indigo-50 border-indigo-200 dark:bg-indigo-900/50 dark:border-indigo-700' : 'bg-white border-transparent hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700'}\`}
                                    >
                                        <div className="flex justify-between items-start">
                                            <div className="overflow-hidden">
                                                ...
                                                <p className="text-xs text-gray-500 dark:text-gray-400">{new Date(d.updatedAt).toLocaleDateString()}</p>
                                            </div>
                                            <button
                ...
                <div className="flex-1 flex flex-row overflow-hidden relative min-w-0">
                    ...
                    <div
                        ...
                        className={\`w-1 h-full cursor-col-resize bg-gray-200 dark:bg-gray-700 hover:bg-indigo-500 transition-colors z-30 flex-none \${isDragging ? 'bg-indigo-500' : ''}\`}
                    />

                    {/* Preview Panel */}
                    <div 
                        className="h-full flex flex-col relative"
                        style={{ width: \`\${100 - leftPanelWidth}%\`, backgroundColor: theme === 'dark' ? '#1e1e1e' : '#ffffff' }}
                    >
                        {renderError && (
                            <div className="absolute top-4 left-4 right-4 z-10 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded shadow-sm">
                                <p className="text-red-800 dark:text-red-300 font-semibold text-sm flex items-center gap-2">
                                    <span className="text-red-600">⚠️</span> Syntax Error
                                </p>
