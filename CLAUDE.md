...
| Date | Change |
|------|--------|
| 2026-02-09 | **Session Persistence** - Chat sessions persisted to DB (`WorkspaceChatSession` model), session list sidebar with resume/rename, dedicated persist endpoint. Terminal sessions never auto-delete (10-year expiry), removed cleanup of DB rows and workspace dirs, migration extends existing sessions. Transcript download endpoint, recording download param. **Audit fixes:** removed double persistence on chat message (server saves metadata only, frontend owns message snapshots), added LRU cache eviction (1h idle) for ChatManager in-memory sessions, fixed DELETE ownership check to fall back to DB, added pagination (limit/offset) to terminal session listing endpoints |
